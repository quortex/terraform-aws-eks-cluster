...
locals {
  handle_iam_aws_load_balancer_controller = var.handle_iam_resources && var.handle_iam_aws_load_balancer_controller
}

resource "aws_iam_role" "aws_load_balancer_controller" {
  count       = local.handle_iam_aws_load_balancer_controller ? 1 : 0
  name        = var.aws_load_balancer_controller_role_name
  description = "IAM Role required for aws-load-balancer-controller."
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            ...
            "${local.cluster_oidc_issuer}:sub" : "system:serviceaccount:${var.aws_load_balancer_controller_sa.namespace}:${var.aws_load_balancer_controller_sa.name}"
          }
        }
      }
    ]
